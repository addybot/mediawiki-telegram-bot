  ...
  if (cached) {
    ...
    if (title) {
      wiki.getArticle(title, (err, result) => {
        if (err) {
          log.error('wiki.getArticle', err);
          bot.sendMessage(fromId, 'Something goes wrong :(');
          return;
        }
        pdc(result, 'mediawiki', 'markdown', (err, result) => {
          if (err) {
            log.error('Error ocurred when converting "%s"', title, err);
            let text = 'Sorry, I can\'t convert this article :(';
            cache.set(id, [text]);
